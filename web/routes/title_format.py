...
Regra de obrigatoriedade dinâmica:
  "Título do Evento" só trava (não pode desligar) quando for o ÚNICO
  componente ativo. Se houver outro ligado, ele pode ser desligado.
"""
...
_JS = Script("""
    ...
    /* Atualiza trava dinâmica: se só 1 ativo, ele não pode ser desligado */
...
def _comp_row(comp: str, label: str, desc: str, enabled: bool, use_brackets: bool) -> Li:
    """Linha DnD de um componente: handle | info | [toggle ligado] [toggle []] """
    ...
    return Li(
        ...
        Div(
            ...
            # Toggle colchetes [ ] — aparece apenas quando ligado
            Button(
                "[ ]",
                type="button",
                cls=bracket_cls,
...
def title_format_page(config, saved: bool = False):
    ...
    for comp in order_list:
        ...
        # "title" começa ligado se não houver nenhum outro (segurança)
