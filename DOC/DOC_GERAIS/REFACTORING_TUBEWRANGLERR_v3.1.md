...
6. Em caso de dúvida, registrar em DECISIONS.md e aguardar — nunca assumir silenciosamente
...
# Verificar permissão sem sudo
docker ps
# Esperado: lista (mesmo vazia), SEM erro de permissão
# Se houver erro: sudo usermod -aG docker $USER  (fazer logout/login após)
```
...
Antes de criar qualquer arquivo, o agente deve conhecer TODAS as dependências do projeto:
...
#### Dependências de sistema (instaladas via apt no Dockerfile)

| Pacote | Motivo | Binário gerado |
|---|---|---|
| `ffmpeg` | Renderizar placeholder/thumbnail com texto overlay | `/usr/bin/ffmpeg` |
| `fonts-dejavu-core` | Fonte para texto no ffmpeg (`DejaVuSans-Bold.ttf`) | `/usr/share/fonts/truetype/dejavu/` |
| `curl` | Health check e utilitários de rede | `/usr/bin/curl` |
...
# Banco de dados de configuração (substitui .env)
fastlite>=0.0.9
...
# Streams ao vivo — usado por smart_player.py como subprocesso
streamlink

# VOD e download — usado por smart_player.py como subprocesso
yt-dlp
...
# ffmpeg: renderização de placeholder e thumbnail com overlay de texto (smart_player.py)
# fonts-dejavu-core: fonte DejaVuSans-Bold.ttf usada pelo ffmpeg
# curl: health check e utilitários
...
# --- Dependências Python ---
# requirements.txt inclui: fasthtml, fastlite, google-api, streamlink, yt-dlp, pytest
COPY requirements.txt .
RUN pip install --no-cache-dir -r requirements.txt

# Código copiado em produção; montado como volume em desenvolvimento
COPY . .

# Garante diretórios de dados dentro do container
# IMPORTANTE: NUNCA criar esses diretórios no host com mkdir
# O volume Docker monta ./data:/data automaticamente
...
### 0.5 docker-compose.yml (produção)
...
### 0.8 Sequência de inicialização (UMA ÚNICA VEZ)

```bash
# Build da imagem (instala apt + pip dentro do container)
...
# ── Verificações obrigatórias de runtime ──
...
# Dependências Python
docker compose exec tubewranglerr pip list | grep -E "fasthtml|fastlite|streamlink|yt-dlp|pytest"
# Esperado: linhas com todos esses pacotes

# ffmpeg (obrigatório para smart_player.py)
docker compose exec tubewranglerr ffmpeg -version
# Esperado: ffmpeg version ...

# streamlink (obrigatório para smart_player.py — streams ao vivo)
docker compose exec tubewranglerr streamlink --version
# Esperado: streamlink 6.x.x

# yt-dlp (obrigatório para smart_player.py — VODs)
docker compose exec tubewranglerr yt-dlp --version
# Esperado: 2024.xx.xx

# Fonte DejaVu (obrigatória para overlay de texto no ffmpeg)
docker compose exec tubewranglerr ls /usr/share/fonts/truetype/dejavu/ | grep Bold
# Esperado: DejaVuSans-Bold.ttf

# Volume /data montado
docker compose exec tubewranglerr ls -la /data
# Esperado: m3us/  epgs/  logs/
```

### 0.9 Padrão de execução — TODO comando Python usa este formato
...
### 0.10 Checklist Etapa 0

```
[ ] requirements.txt criado (com streamlink, yt-dlp — sem Flask, sem python-dotenv)
[ ] Dockerfile criado (ffmpeg + fonts-dejavu-core + streamlink + yt-dlp)
...
[ ] docker compose exec tubewranglerr python3 --version → 3.12.x
[ ] docker compose exec tubewranglerr pip list → fasthtml, fastlite, streamlink, yt-dlp presentes
[ ] docker compose exec tubewranglerr ffmpeg -version → retorna versão
[ ] docker compose exec tubewranglerr streamlink --version → retorna versão
[ ] docker compose exec tubewranglerr yt-dlp --version → retorna versão
[ ] docker compose exec tubewranglerr ls .../dejavu/ → DejaVuSans-Bold.ttf presente
[ ] docker compose exec tubewranglerr ls -la /data → m3us/ epgs/ logs/ presentes
[ ] DECISIONS.md criado com tabela de status e decisão de ambiente
```
...
PROIBIDO: Criar /data com mkdir no host — o volume Docker gerencia automaticamente
...
OBRIGATÓRIO: Toda decisão não explícita neste documento registrada no DECISIONS.md
...
O fastlite retorna rows como dicionários, não objetos com atributos.
O código abaixo já incorpora essa correção. **Não alterar o padrão de acesso.**
...
assert total == 43, f'ERRO: esperava 43, tem {total}'
print(f'✅ AppConfig OK — {total} chaves em {list(sections.keys())}')
"
```
...
[ ] Todas as 43 chaves presentes no DEFAULTS
...
### 4.1 Ordem obrigatória de criação
...
### 4.3 Validação da Etapa 2

```bash
docker compose exec tubewranglerr python3 -c "from core.state_manager import StateManager; print('✅')"
docker compose exec tubewranglerr python3 -c "from core.youtube_api import YouTubeAPI; print('✅')"
docker compose exec tubewranglerr python3 -c "from core.playlist_builder import M3UGenerator; print('✅')"
docker compose exec tubewranglerr python3 -c "from core.scheduler import Scheduler; print('✅')"
...
[ ] Todos os testes da etapa passam
...
### 5.3 Padrão componentes FastHTML

```python
# ✅ CORRETO — componente retorna FT
...
[ ] GET / retorna 200
[ ] GET /config mostra 6 seções
[ ] POST /config aplica sem restart
[ ] GET /channels funcional
[ ] GET /logs SSE funcional
[ ] M3U com mimetype application/vnd.apple.mpegurl
[ ] EPG com mimetype application/xml
...
```bash
# Sem o override (simula produção)
...
# Verificar que streamlink e yt-dlp estão disponíveis em produção
docker compose exec tubewranglerr streamlink --version
docker compose exec tubewranglerr yt-dlp --version
```

### 6.2 Checklist Etapa 4

```
[ ] docker compose -f docker-compose.yml build → sem erro
[ ] docker compose -f docker-compose.yml up -d → sobe
...
[ ] streamlink disponível no container de produção
[ ] yt-dlp disponível no container de produção
[ ] DECISIONS.md atualizado
```
...
### 7.1 Contexto do smart_player.py

O `smart_player.py` é um roteador que recebe uma URL e decide como reproduzir:
- **Stream ao vivo** → chama `streamlink` como subprocesso
- **VOD/gravado** → chama `yt-dlp` como subprocesso
- **Thumbnail/placeholder** → chama `ffmpeg` como subprocesso (overlay de texto)

Esses três binários são instalados no container via Dockerfile. A única mudança
necessária é substituir `os.getenv()` e `load_dotenv()` por `AppConfig`.

### 7.2 Mudanças — before/after exato
...
TEXTS_CACHE_PATH = SCRIPT_DIR / "textosepg.json"
...
### 7.3 Verificação de binários (não alterar — já correto)

O `smart_player.py` original já verifica `FileNotFoundError` ao chamar
`streamlink`, `yt-dlp` e `ffmpeg`. Esses blocos **não devem ser alterados**:

```python
# Manter como está — o container garante que os binários existem
except FileNotFoundError:
    logger.error("Erro: Comando ffmpeg não encontrado.")
except FileNotFoundError:
    logger.error("Erro: Comando streamlink não encontrado.")
except FileNotFoundError:
    logger.error("Erro: Comando yt-dlp não encontrado.")
```

### 7.4 Validação da Etapa 5
...
# Verificar que streamlink funciona (teste real)
docker compose exec tubewranglerr streamlink --version

# Verificar que yt-dlp funciona
docker compose exec tubewranglerr yt-dlp --version

# Verificar que ffmpeg com DejaVu funciona
docker compose exec tubewranglerr ffmpeg -f lavfi -i testsrc=size=640x480 -frames:v 1 /tmp/test.jpg -y 2>&1 | tail -1
```

### 7.5 Checklist Etapa 5
...
[ ] grep load_dotenv/os.getenv smart_player.py retorna vazio
[ ] streamlink --version OK no container
[ ] yt-dlp --version OK no container
[ ] ffmpeg -version OK no container
[ ] DECISIONS.md atualizado
```
...
def test_adiciona_stream(state):
    state.update_streams([{"video_id": "abc123", "status": "live",
                           "channel_id": "ch1", "title_original": "Test"}])
    assert "abc123" in state.streams

def test_save_e_load(state, tmp_path, cfg):
...
def test_config_post(client):
    assert client.post("/config", data={"http_port": "9000"}).status_code in (200,302,303)
...
[ ] POST /config aplica sem restart
[ ] Playlists nas URLs corretas
[ ] smart_player.py sem load_dotenv
[ ] state_cache.json e textosepg.json lidos de /data/
[ ] streamlink, yt-dlp e ffmpeg disponíveis no container
```
...
sha256sum get_streams.py file.env  # registrar no DECISIONS.md
git add .
...
Ambiente: VS Code Windows → SSH → Debian → Docker (python:3.12-slim)
...
**Contexto:** VS Code Windows → SSH → Debian → Docker
**Decisão:** Nenhum Python, ffmpeg, streamlink ou yt-dlp roda no host.
...
```markdown
# Copilot Instructions — TubeWranglerr v3.1
...
- NUNCA instalar Python, pip, ffmpeg, streamlink ou yt-dlp no host Debian
...
- Streams ao vivo: streamlink (subprocesso — instalado no container)
- VOD: yt-dlp (subprocesso — instalado no container)
- Thumbnails/placeholder: ffmpeg + fonts-dejavu-core (instalados no container)
...
*Versão 3.1 — Dependências completas: streamlink + yt-dlp + ffmpeg + DejaVu fonts no Dockerfile. Container-first, fastlite como dicionário, sem instalação no host.*
